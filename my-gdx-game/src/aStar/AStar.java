...
public class AStar {
	...
	private Path shortestPath;//el camino a seguir

	public AStar() {
		// TODO Auto-generated constructor stub
	}
	
	public void ReadTheStars(AreaMap map, AStarHeuristic heuristic) {
		this.map = map;
		...
		while(openList.size() != 0) {
			...
			for(Node neighbor : current.getNeighborList()) {
				...
				if (!neighbor.isObstacle) {
					...
					} else if(neighborDistanceFromStart < current.getDistanceFromStart()) {//si la distancia del vecino es menor, es mejor
						neighborIsBetter = true;
					} else {
	...
	private Path reconstructPath(Node node) //crea el camino desde la meta al inicio, siguiendo los hijos
	{
		Path path = new Path();
